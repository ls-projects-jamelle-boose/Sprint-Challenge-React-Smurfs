...
- [ ] Explain the differences between `client-side routing` and `server-side routing`.
      **Client side routing stores the info for the app on the client and only updates in sections and only when needed. You may have to download alot of info though. Server side routing is easier for webcrawlers to read, but you have to make a call to the server everytime data changes.**
